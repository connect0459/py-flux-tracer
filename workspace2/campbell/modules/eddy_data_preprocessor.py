...
class EddyDataPreprocessor:
    ...
    def add_uvw_columns(
        ...
        for key in required_keys:
            if key not in column_mapping:
                raise ValueError(
                    f"column_mapping に必要なキー '{key}' が存在しません。"
                )
        ...
        for filename in tqdm(csv_files, desc="Processing files"):
            ...
            if output_c2c1_ratio:
                ...
                ratio_row: dict[str, str | float] = {
                    ...
                    "ols_slope": f"{np.nan}",
                    "ols_intercept": f"{np.nan}",
                    "ols_r_value": f"{np.nan}",
                    "ols_p_value": f"{np.nan}",
                    "ols_stderr": f"{np.nan}",
                    "tls_slope": f"{np.nan}",
                    "tls_intercept": f"{np.nan}",
                    "tls_r_value": f"{np.nan}",
                }

                try:
                    # OLS回帰を計算
                    ols_slope, ols_intercept, ols_r_value, ols_p_value, ols_stderr = stats.linregress(
                        ch4_data, c2h6_data
                    )
                    
                    # TLS回帰を計算
                    tls_slope, tls_intercept, tls_r_value = EddyDataPreprocessor._calculate_tls_regression(
                        ch4_data.to_numpy(), c2h6_data.to_numpy()
                    )

                    ratio_row.update({
                        "Date": start_time.strftime("%Y-%m-%d %H:%M:%S.%f"),
                        "ols_slope": f"{ols_slope:.6f}",
                        "ols_intercept": f"{ols_intercept:.6f}",
                        "ols_r_value": f"{ols_r_value:.6f}",
                        "ols_p_value": f"{ols_p_value:.6f}",
                        "ols_stderr": f"{ols_stderr:.6f}",
                        "tls_slope": f"{tls_slope:.6f}",
                        "tls_intercept": f"{tls_intercept:.6f}",
                        "tls_r_value": f"{tls_r_value:.6f}",
                    })
                except Exception as e:
                    self.logger.warning(f"回帰計算中にエラーが発生しました: {e!s}")
                    # デフォルトのratio_rowを使用
    ...
    @staticmethod
    def _calculate_tls_regression(
        x: np.ndarray, y: np.ndarray
    ) -> tuple[float, float, float]:
        """Total Least Squares (TLS)回帰、または直交回帰を計算する関数。
        ...
        理論:
        1. データを中心化(平均を0に)します
        2. 中心化データから2*N行列を作成します
        3. 特異値分解(SVD)を実行: M = USV^T
        4. 最小特異値に対応する特異ベクトル v = (v1, v2) から、
           傾き slope = -v1/v2 を得ます
        5. 切片は中心化の補正から: intercept = y_mean - slope * x_mean
        ...
        References
        ----------
        .. [1] Golub, G. H., & Van Loan, C. F. (1980). An analysis of the total
               least squares problem. SIAM Journal on Numerical Analysis, 17(6), 883-893.
        """
        # データを中心化
        x_mean = np.mean(x)
        y_mean = np.mean(y)
        ...
        # データ行列を作成
        data_matrix = np.vstack((x_centered, y_centered))

        # 特異値分解を実行
        _, _, v_t = np.linalg.svd(data_matrix)

        # 最小特異値に対応する右特異ベクトルを使用
        v = v_t[-1]
