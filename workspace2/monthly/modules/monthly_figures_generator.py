...
class MonthlyFiguresGenerator:
    ...
    def plot_source_contributions_diurnal_with_slope(
        self,
        df: pd.DataFrame,
        col_ch4_flux: str,
        col_c2h6_flux: str,
        col_slope: str,
        col_datetime: str = "Date",
        color_bio_flux: str = "blue",
        color_gas_flux: str = "red",
        color_bio_conc: str = "darkblue",
        color_gas_conc: str = "darkred",
        label_bio_flux: str = "生物(フラックス)",
        label_gas_flux: str = "都市ガス(フラックス)",
        label_bio_conc: str = "生物(濃度)",
        label_gas_conc: str = "都市ガス(濃度)",
        flux_alpha: float = 0.6,
        output_dirpath: str | Path | None = None,
        output_filename: str = "source_contributions_with_slope.png",
        ...
        """CH4フラックスの起源分離を2つの方法(フラックスベースと濃度勾配ベース)で計算し表示します。
        ...
        ----------
            ...
            col_slope: str
                C2H6/CH4濃度勾配のカラム名
            col_datetime: str, optional
                日時カラムの名前。デフォルトは"Date"
            color_bio_flux: str, optional
                生物起源(フラックスベース)の色。デフォルトは"blue"
            color_gas_flux: str, optional
                都市ガス起源(フラックスベース)の色。デフォルトは"red"
            color_bio_conc: str, optional
                生物起源(濃度ベース)の色。デフォルトは"darkblue"
            color_gas_conc: str, optional
                都市ガス起源(濃度ベース)の色。デフォルトは"darkred"
            label_bio_flux: str, optional
                生物起源(フラックスベース)のラベル。デフォルトは"生物(フラックス)"
            label_gas_flux: str, optional
                都市ガス起源(フラックスベース)のラベル。デフォルトは"都市ガス(フラックス)"
            label_bio_conc: str, optional
                生物起源(濃度ベース)のラベル。デフォルトは"生物(濃度)"
            label_gas_conc: str, optional
                都市ガス起源(濃度ベース)のラベル。デフォルトは"都市ガス(濃度)"
            flux_alpha: float, optional
                フラックスの透明度。デフォルトは0.6
            output_dirpath: str | Path | None, optional
                保存先のディレクトリ。save_fig=Trueの場合は必須
            output_filename: str, optional
                保存するファイル名。デフォルトは"source_contributions_with_slope.png"
            gas_ratio_c1c2: float, optional
                都市ガスのC2H6/CH4比(無次元)。デフォルトは0.076
            add_legend : bool, optional
        ...
        >>> mfg.plot_source_contributions_diurnal_with_slope(
        ...     df=df,
        ...     col_ch4_flux="Fch4_ultra",
        ...     col_c2h6_flux="Fc2h6_ultra",
        ...     col_slope="slope",
        ...
        # 濃度勾配から都市ガス比率を計算
        df_with_sources["gas_ratio_slope"] = df[col_slope] / gas_ratio_c1c2 * 100
        # 0-100%の範囲外をNaNに設定
        df_with_sources.loc[
            (df_with_sources["gas_ratio_slope"] < 0)
            | (df_with_sources["gas_ratio_slope"] > 100),
            "gas_ratio_slope",
        ] = np.nan

        # 濃度勾配ベースのCH4フラックス分離
        df_with_sources["ch4_gas_slope"] = df_with_sources[col_ch4_flux] * np.clip(
            df_with_sources["gas_ratio_slope"] / 100, 0, 1
        )
        df_with_sources["ch4_bio_slope"] = df_with_sources[col_ch4_flux] * (
            1 - np.clip(df_with_sources["gas_ratio_slope"] / 100, 0, 1)
        )

        # 時刻データの抽出とグループ化
        df_with_sources["hour"] = df_with_sources.index.hour
        hourly_means = df_with_sources.groupby("hour")[
            ["ch4_gas", "ch4_bio", "ch4_gas_slope", "ch4_bio_slope"]
        ].mean()
        ...
        ax1.fill_between(
            ...
            0,
            hourly_means["ch4_bio"],
            color=color_bio_flux,
            alpha=flux_alpha,
            label=label_bio_flux,
        )
        ax1.fill_between(
            time_points,
            hourly_means["ch4_bio"],
            hourly_means["ch4_bio"] + hourly_means["ch4_gas"],
            color=color_gas_flux,
            alpha=flux_alpha,
            label=label_gas_flux,
        )

        # 濃度勾配ベースのプロット(線のみ)
        ax1.plot(
            time_points,
            hourly_means["ch4_bio_slope"],
            "-",
            color=color_bio_conc,
            linewidth=2,
            label=label_bio_conc,
        )
        ax1.plot(
            time_points,
            hourly_means["ch4_bio_slope"] + hourly_means["ch4_gas_slope"],
            "-",
            color=color_gas_conc,
            linewidth=2,
            label=label_gas_conc,
        )
        ...
        if add_legend:
            # 凡例を1つの枠内で2行に配置
            handles, labels = ax1.get_legend_handles_labels()
            # フラックスベースと濃度ベースの順序を整理
            order = [
                0,
                2,
                1,
                3,
            ]  # 生物(フラックス), 生物(濃度), 都市ガス(フラックス), 都市ガス(濃度)
            handles = [handles[i] for i in order]
            labels = [labels[i] for i in order]

            # 2行分の凡例のために、より大きな余白を確保
            plt.subplots_adjust(bottom=0.35)  # bottom

            fig.legend(
                handles,
                labels,
                loc="center",
                bbox_to_anchor=(0.5, -0.05),  # x座標、y座標
                ncol=2,  # 2列表示
            )
        plt.tight_layout()
        ...
        if save_fig:
            if output_dirpath is None:
                raise ValueError(
                    "save_fig = True の場合、output_dirpathを指定する必要があります。"
                )
            os.makedirs(output_dirpath, exist_ok=True)
            output_filepath: str = os.path.join(output_dirpath, output_filename)
            plt.savefig(output_filepath, dpi=dpi, bbox_inches="tight")
    ...
    def _aggregate_direction_data(
        ...
        for idx, row in direction_ranges.iterrows():
            ...
            result_data.at[idx, "gas_flux"] = np.nanmean(
                np.array(df_internal.loc[mask, "ch4_gas"]).astype(float)
            )
            result_data.at[idx, "bio_flux"] = np.nanmean(
                np.array(df_internal.loc[mask, "ch4_bio"]).astype(float)
            )
        # NaNを0に置換
